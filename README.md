...
- **完成通知**：任务完成 / 出错 / 等待确认时弹 **Windows 系统通知**（带用时与步骤统计，点通知唤起桌宠）——桌宠被窗口挡住或已隐藏时也不会错过
- **点行跳转**：任务面板里点任意一行，直接把该应用的窗口切到前台，不用再去任务栏找
- **停用只是忽略**：点「停用」只是让桌宠忽略该应用的事件，卡片仍然留在设置里（随时可一键启用），已装的钩子也不会被动
...
**更省事的做法**：设置 → 应用集成 → 点「🔄 重新扫描应用」——PetBuddy 会认出本机装过的
